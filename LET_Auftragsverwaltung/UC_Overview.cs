...
namespace LET_Auftragsverwaltung
{
    public partial class UC_Overview : UserControl
    {
        ...
        public UC_Overview( )
        {
            ...
            while (reader.Read())
            {
                data.Auftrags_Nr = ( string ) reader["Auftrags Nr."];
                data.Fertigungsstatus = ( string ) reader["Status"];
                data.Erstell_Datum = Convert.ToDateTime(reader["Erstelldatum"]);
                data.Anzahlung_Datum = Convert.ToDateTime(reader["Anzahlung anfordern"]);
                data.AZ_bestaetigt_Datum = Convert.ToDateTime(reader["Anzahlung bestätigt"]);
                data.Schlussrechnung_Date = Convert.ToDateTime(reader["Schlussrechnung ausgestellt am"]);
                data.Projektverantwortlicher_Name = ( string ) reader["Projektverantwortlicher"];
                data.Planner_Name = ( string ) reader["Planner"];
                data.Projektbezeichnung = ( string ) reader["Projektbezeichnung"];
                data.Auftrags_Art = ( string ) reader["Auftrags Art"];
                data.Stoff_Kennzahl = ( string ) reader["Stoff"];
                data.Schatten_Datum = Convert.ToDateTime(reader["Schatten fertig"]);
                data.Stoff_bestell_Datum = Convert.ToDateTime(reader["Stoff Bestelldatum"]);
                data.Stoff_liefer_Datum = Convert.ToDateTime(reader["Stoff Lieferdatum"]);
                data.Stoff_Lieferant = ( string ) reader["Stoff Lieferant"];
                data.Sonderteile_bestell_Datum = Convert.ToDateTime(reader["Sonderteile Bestelldatum"]);
                data.Sonderteile_liefer_Datum = Convert.ToDateTime(reader["Sonderteile Lieferdatum"]);
                data.Sonderteile_Lieferant = ( string ) reader["Sonderteile Lieferant"];
                data.Persenning_bestell_Datum = Convert.ToDateTime(reader["Persenning Bestelldatum"]);
                data.Persenning_liefer_Datum = Convert.ToDateTime(reader["Persenning Lieferdatum"]);
